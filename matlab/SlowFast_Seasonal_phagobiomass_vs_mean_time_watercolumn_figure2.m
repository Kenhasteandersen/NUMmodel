...
mortHTL=0.4; %BURDE VÆRE 0.5. KØR 0.5 IGEN
...
%% load simulation
load(fullfile('ModelResults','watercolumn','seasonal',['workspace_latlon_',num2str(latlon(1)),'_',num2str(latlon(2)),'_mortHTL_0_',num2str(10*mortHTL),'.mat']));
% sim.B=B_osmo+B_phago+B_photo;
% set  small values to minval
sim.B(sim.B<0.01)=minval;

%% Plot the different alpha and palatability groups
figure('Color','w');
cm = parula(length(p.nameGroup)-1);
alphamin=-4;
alphamax=0;
alphanrintervals=1000;
[v,~,alpha,~]=getVulnformulation(alphamin,alphamax,alphanrintervals);
plot(alpha,v,':','Color',[0.8 0.8 0.8],'LineWidth',1.5)
hold on
scatter(alphaF_these,palatability_these,60,cm,'filled')
set(gca,'XScale','log');
xlabel('Specific clerance rate (l day^{-1} \mugC^{-1})')
ylabel('Predation risk')
box off
ax=gca;
ax.LineWidth=1.5;
ax.FontSize=12;
...
B_interped=nan(length(t),size(sim.B,2),size(sim.B,3));
B_phag_interped=B_interped;
B_photo_interped=B_interped;
B_osmo_interped=B_interped;
for j=1:size(sim.B,2) % for all depths
    for i=1:size(sim.B,3) % for all groups
        B_interped(:,j,i)=interp1(sim.t(ix),sim.B(ix,j,i),t);
        B_phag_interped(:,j,i)=interp1(sim.t(ix),B_phag(:,j,i),t);
        B_photo_interped(:,j,i)=interp1(sim.t(ix),B_photo(:,j,i),t);
        B_osmo_interped(:,j,i)=interp1(sim.t(ix),B_osmo(:,j,i),t);
    end
end
...
%%
BiomassAcrossTheYearAndDepth(B_interped,sim,p,alphaF_these,t_mod,'Total biomass across year and depth')
%%
...
%%
for j=1:size(B_interped,2)
    for ii=2:length(sim.p.nameGroup)
        for itime=1:365
            this_time=find(t_mod==itime);
            theseB=B_interped(this_time,j,p.ixStart(ii)-2:p.ixEnd(ii)-2);
            % % take the mean of all dates
            B_meanYaar(itime,j,1:20,ii)=mean(theseB,1,'omitnan');
            B_minYaar(itime,j,1:20,ii)=min(theseB);
            B_maxYaar(itime,j,1:20,ii)=max(theseB);



            theseB_phag=B_phag_interped(this_time,j,p.ixStart(ii)-2:p.ixEnd(ii)-2);
            % % take the mean of all dates
            B_phag_meanYaar(itime,j,1:20,ii)=mean(theseB_phag,1,'omitnan');
            B_phag_minYaar(itime,j,1:20,ii)=min(theseB_phag);
            B_phag_maxYaar(itime,j,1:20,ii)=max(theseB_phag);

            theseB_photo=B_photo_interped(this_time,j,p.ixStart(ii)-2:p.ixEnd(ii)-2);
            B_photo_meanYaar(itime,j,1:20,ii)=mean(theseB_photo,1,'omitnan');
            B_photo_minYaar(itime,j,1:20,ii)=min(theseB_photo);
            B_photo_maxYaar(itime,j,1:20,ii)=max(theseB_photo);

            theseB_osmo=B_osmo_interped(this_time,j,p.ixStart(ii)-2:p.ixEnd(ii)-2);
            B_osmo_meanYaar(itime,j,1:20,ii)=mean(theseB_osmo,1,'omitnan');
            B_osmo_minYaar(itime,j,1:20,ii)=min(theseB_osmo);
            B_osmo_maxYaar(itime,j,1:20,ii)=max(theseB_osmo);
        end
    end
end

%% Plot biomass for spring
whichyear=linspace(94,335,8);
    ...
    for ii=1:length(whichyear)
        ...
        if ~isempty(these)
            for iii=1:length(these)
                ...
                text((mUpper(these(iii))+mlower(these(iii)))./2,ii,num2str(xx(these(iii))),'Rotation',90)
            end
        end
    end
end
...
for j=1:size(B,2)
    ...
    for ii=2:length(sim.p.nameGroup)
        for itime=1:365
            ...
            theseB_phag=squeeze(B(this_time,j,p.ixStart(ii)-2:p.ixEnd(ii)-2));
    ...
    title(['depth nr',num2str(sim.z(j))])
end
f=gcf;
for i=1:length(f.Children.Children)
    nexttile(i)
    ylim([0 max(themax)])
    xlabel('Days of year');
...
function BiomassAcrossTheYearAndDepth(B,sim,p,alphaF,t_mod,thistitle)
% Function that plots the biomass as a function of day in the year. It
% plots all the different alphaF groups on their own.
figure('Color','w')
cm = parula(length(sim.p.nameGroup)-1);
...
for ii=2:length(sim.p.nameGroup)
    ...
    for itime=1:365
        ...
        B_mean(:,itime)=mean(sum(theseB_phag,3,'omitnan'))';
        % B_mean(:,itime)=(sum(theseB_phag,3,'omitnan'))';
    end
    [x,y]=meshgrid(1:size(B_mean,2),sim.z);
    contourf(x,y,B_mean,500,'LineStyle','none');set(gca,'YDir','reverse')
    title(['group number',num2str(ii-1)])
    colorbar
end
end
...
cm = parula(length(sim.p.nameGroup)-1);
B_mean=nan(365,12,20,length(sim.p.nameGroup));
B_min=B_mean;
B_max=B_mean;
...
for j=1:size(B,2)
    for ii=2:length(sim.p.nameGroup)
        for i=1:365
            iix=t_mod==i;
            theseB_photo=squeeze(B(iix,j,p.ixStart(ii)-2:p.ixEnd(ii)-2));
            % get the mean, min and max of all "samples" from this day:
            B_mean(i,j,1:20,ii)=mean(theseB_photo,1,'omitnan');
            B_min(i,j,1:20,ii)=min(theseB_photo);
            B_max(i,j,1:20,ii)=max(theseB_photo);
        end
    end
end
%%
figure('color','w');
t=tiledlayout(size(B,2),length(whichyear),'TileIndexing','rowmajor');
[x,y]=meshgrid(p.m(p.ixStart(2):p.ixEnd(2)),alphaF_these);
tnr=0;
for j=1:size(B,2)
    for ii=1:length(whichyear)
        tnr=tnr+1;
        nexttile(tnr)
        thisyear=whichyear(ii);
        pcolor(x,y,squeeze(B_mean(round(thisyear),j,:,2:end))')
        set(gca,'XScale','log','YScale','log');
        if j==1
            title(['day ',num2str(round(thisyear))])
        end
        if ii==1
            ylabel(['depth',num2str(sim.z(j))])
        end
        colorbar
    end
end
%%
for j=1:size(B,2)
    figure('Color','w','Name',[titletext,' for depth ',num2str(sim.z(j))])
    ...
    for ii=1:length(whichyear)
        ...
        for i=2:length(sim.p.nameGroup)
            ...
            inBetween = [squeeze(B_min(round(thisyear),j,:,i)); fliplr(squeeze(B_max(round(thisyear),j,:,i)))];
            ...
            Bio=squeeze(B_mean(round(thisyear),j,:,i))';
            [Bio]=removeTooSmallPoints(Bio,0.5);
            TF = islocalmin(Bio);
            % ADD NUMBERS
            biophag=squeeze(B_phag_meanYaar(round(thisyear),j,:,i))';
            biophot=squeeze(B_photo_meanYaar(round(thisyear),j,:,i))';
            bioosmo=squeeze(B_osmo_meanYaar(round(thisyear),j,:,i))';
            xx=(1-((biophot+bioosmo)./(biophag+biophot+bioosmo)));
            bio=squeeze(B_meanYaar(round(thisyear),j,:,i))';
            [bio]=removeTooSmallPoints(bio,0.5);
            xx(isnan(bio))=NaN;
            these=find(~isnan(xx));
