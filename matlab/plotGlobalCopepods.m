...
nexttile
handle = panelGlobal(sim.x,sim.y,log10(squeeze(sum(Bgroup,1))),[-2 2],sProjection=options.sProjection,...
    sUnits='g_C/m^2');


% Active/passive:
nexttile
panelGlobal(sim.x, sim.y, Bgroup(2,:,:)./(Bgroup(1,:,:)+Bgroup(2,:,:)),[0 1],sProjection=options.sProjection,...
