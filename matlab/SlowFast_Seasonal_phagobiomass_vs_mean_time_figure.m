...
BiomassAcrossSpring(B_interped,whichyear,sim,p,marklines,alphaF_these,t_mod,'Spring bloom for total biomass')
...
%% Hvad er ratio mellem de to? 1 fuldt plot
marklines=(0.4*1d6*1d-12).*(4/3).*pi.*[2 20].^(3);
whichyear=linspace(49,131,8);
figure('Color','w','Name','ratio biomass')
xmat=repmat(p.m(p.ixStart(2):p.ixEnd(2)),[length(whichyear) 1]);
ymat=repmat((1:length(whichyear))',[1 length(p.m(p.ixStart(2):p.ixEnd(2)))]);
for i=2:length(sim.p.nameGroup)
    subplot(length(sim.p.nameGroup)-1,1,i-1)
    for ii=1:length(whichyear)
        thisyear=whichyear(ii);
        zz=squeeze(B_phag_meanYaar(round(whichyear),:,i))';
        surf(xmat,ymat,flipud(zz'),'FaceAlpha','flat','LineStyle','none',...
...
'FaceColor',cm(i-1,:))
        ...
        hold on 
        cf=contourf(xmat,ymat,flipud(zz'),'LineStyle','none');
        set(gca,'XScale','log')
        xlim([p.m(p.ixStart(2)) p.m(p.ixEnd(2))])
        ylim([1 20])
        clim([0 1])
        % cc=colorbar
        ...
        %     biophag(biophag==0.01)=0;
        %     biophot(biophot==0.01)=0;
        %     biophot(biophag==0)=0;
        % 
        %     bar(biophag./(biophag+biophot),'FaceColor',cm(i-1,:));
        %     xlim([1 20])
        %     ax=gca;
        %     ax.XTick=1:20;
...
for ii=1:length(whichyear)
    ...
    hold on
    for i=2:length(sim.p.nameGroup)
        biophag=squeeze(B_meanYaar(round(thisyear),:,i))';
        % if any(biophag~=0.01)
        %     nexttile
        %     biophot=squeeze(B_photo_meanYaar(round(thisyear),:,i))';
        ...
        %     biophot(biophot==0.01)=0;
        %     biophot(biophag==0)=0;
        %     bioosmo(bioosmo==0.01)=0;
        %     bioosmo(biophag==0)=0;
        % 
        %     % plot(p.m(p.ixStart(2):p.ixEnd(2)),biophag./(biophag+biophot+bioosmo))
        %     % set(gca,'XScale','log')
        %     %OR
        %     bar(biophag./(biophag+biophot+bioosmo),'FaceColor',cm(i-1,:));
        %     xlim([1 20])
        %     ax=gca;
        %     ax.XTick=1:20;
        %     ax.XTickLabel=p.m(p.ixStart(2):p.ixEnd(2));
        ...
        if any(biophag~=0.01)
            ...
            % plot(p.m(p.ixStart(2):p.ixEnd(2)),1-((biophot+bioosmo)./(biophag+biophot+bioosmo)),'Color',cm(i-1,:),'DisplayName',['Generalist \alpha=',num2str(alphaF(i-1))])
            % set(gca,'XScale','log')
            %OR
            bar(biophag./(biophag+biophot+bioosmo),'FaceColor',cm(i-1,:));
            xlim([1 20])
...
function BiomassAcrossSpring(B,whichyear,sim,p,marklines,alphaF_these,t_mod,titletext)
cm = parula(length(sim.p.nameGroup)-1);
