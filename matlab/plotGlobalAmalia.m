...
%
function plotGlobalAmalia(sim,idepth, iTime, options)
...
tiledlayout(round((2+bSilicate+sim.p.nGroups)/2),4)
% DOC
...
for i = 1:sim.p.nGroups
    ...
    cbar = panelGlobal(sim.x,sim.y,log10(field),...
        [0 1], sTitle=strcat('Depth-int log_{10}(',sim.p.nameGroup(i),')'), sProjection=sProjection);
    clim([0 1])
...
for i = 1:sim.p.nGroups
    nexttile
    field = squeeze(sum(sim.B(:,:,:,idepth,(sim.p.ixStart(i):sim.p.ixEnd(i))-sim.p.idxB+1),5));
    % field = calcIntegrateGlobal(sim,field,bAverageTime);
      %is it really surface though?
    field = squeeze(mean(field));
    M=max(field,[],'all');
    cbar = panelGlobal(sim.x,sim.y,(field),...
        [0 M], sTitle=strcat('Annual average surface (',sim.p.nameGroup(i),')'), sProjection=sProjection);
    clim([0 M])
    cbar.Label.String  = '\mug_C/l';
    set(gca,'XTickLabel','')
