...
mortloss = sum(B.*((1-p.pGeneralists.remin2)*mort2 + p.mortHTLm(ix)));

dNdt   =  sum((-rates.JN(ix)+rates.JNloss(ix)).*B./m)  ...
    + p.pGeneralists.remin2*sum(mort2.*B)/p.pGeneralists.rhoCN  ...
    + p.pGeneralists.remin*mortloss/p.pGeneralists.rhoCN;
...
dDOCdt =  - sum(rates.JDOC(ix).*B./m) + ...
    ...
    p.pGeneralists.remin2*sum(mort2.*B) + ...
    p.pGeneralists.remin*mortloss;
