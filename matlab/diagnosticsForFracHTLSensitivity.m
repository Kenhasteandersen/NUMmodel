...
params={'fracHTL_to_N'};
directory_nm='C:\Users\ampap\OneDrive - Danmarks Tekniske Universitet\Documents\GitHub\NUMmodel\matlab\fracHTL sensitivity\';
...
%                         param            years      depth         months
 monthly_Btot=zeros(length(newParameter),noYears,depth_layers(ilat),12);
 monthly_Ntot=monthly_Btot; monthly_Bcop=monthly_Btot; monthly_Buni=monthly_Btot;
 monthly_Bph=zeros(length(newParameter),noYears,depth_layers(ilat),12);
%                          param                   depth          months 
 monthly_Btot_mean=zeros(length(newParameter),depth_layers(ilat),12);
 monthly_Ntot_mean=monthly_Btot_mean;  monthly_Bcop_mean=monthly_Btot_mean; monthly_Buni_mean=monthly_Btot_mean;
 monthly_Bph_mean=zeros(length(newParameter),depth_layers(ilat),12);
 
 Btot=zeros(length(newParameter),noYears*365,depth_layers(ilat));
 Ntot=Btot;  Bcop=Btot;  Buni=Btot; 

    for ixparam = 1:length(newParameter)    % iterate for different values of the specific parameter
    % ixparam=1;
