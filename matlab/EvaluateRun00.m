...
heightf=16; %figure height in cm
...
% Arctic cruises are not included
    function fit = GOPOPCORNfigures(sim,r)
        ...
        loglog(POCavg_uM,Pbiom_model_final,'o','markerfacecolor',' #a2d9ce','markeredgecolor',' #45b39d','markersize',2);
    ...
    function fit = PicophytoFiguresAtlantic(sim,r)
        ...
        for j=1:length(lat)
            if (lon(j)>=-60 && lon(j)<=0)
                % set(gca,'XTickLabel',[]);
                hold on
                plot(insitu(j),lat(j),'o','markerfacecolor',' #a2d9ce','markeredgecolor',' #45b39d ','markersize',4);
                plot(Picobiom_model(j),lat(j),'o','markerfacecolor','k','markeredgecolor','k','markersize',4);
                % scatter(insitu(j),lat(j),[],'cyan','filled');
                % scatter(Picobiom_model(j),lat(j),'k','filled')
                % ylim(latlim)
...
%
    function [fit, Bmodel] = AMTcopepods(sim)
        ...
        %
        nexttile
        loglog(Btot, Bmodel,'o','markerfacecolor',' #a2d9ce','markeredgecolor',' #45b39d')
        hold on
        plot([0.05 50],[0.05 50],'k-','linewidth',2)
        hold off
        ...
        nexttile
        plot(Btot,lat,'o','markerfacecolor',' #a2d9ce','markeredgecolor','#45b39d')
        hold on
        plot(Bmodel, lat,'o','markerfacecolor','k','markeredgecolor','k')
        %ylabel('Latitude')
        ylim(ylimLatitude)
