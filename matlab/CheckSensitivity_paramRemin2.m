...
lat_to_find=55; lon_to_find=-40;name_png='sensitivityNPP';noYears=4;

% function CheckSensitivity_paramRemin2(lat_to_find,lon_to_find,name_png,NPP_extracted,noYears)
...
newRemin2= logspace(log10(0.01), log10(0.5), 1);
...
for  iparam = 1:length(newRemin2)
    ...
    substituteInputParameters(paramToReplace,InputListName,newvalue)    
        ...
        % save(['simremin2_',num2str(remin2),'lat',num2str(lat_to_find),'.mat'],'sim');    
        % save(['simWCremin2_',num2str(remin2),'lat',num2str(lat_to_find),'.mat'],'simWCSeason');    
...
monthly_Bphyto=zeros(length(newRemin2),noYears,12,length(sim.z));
monthly_Bph_mean=zeros(length(newRemin2),12,length(sim.z));
