...
module NUMmodel
  ...
  !
  ! The group objects hold both parameters and the rates of the grid cell currently
  ! being calculated. To make the library thread safe, each OpenMP thread has
  ! its own copy. The copies are synchronized from the master thread at the start
  ! of each parallel region via groupShared (see simulateEulerCells).
  !
  !$omp threadprivate(group, upositive, F)
  type(spectrumContainer), allocatable :: groupShared(:) ! Snapshot of master's group
  !
...
contains
  ...
  subroutine simulateEulerCells(nCells, u, L, T, tEnd, dt)
    ...
    !
    ! Snapshot the master thread's objects (parameters may have been changed
    ! by setHTL etc. since the last call):
    !
    groupShared = group
